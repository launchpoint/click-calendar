...
$.fn.fullCalendar = function(options) {
	...
	this.each(function() {
		...
		function fetchEvents(callback) {
			events = []; //if (!view.visStart) view.visStart = (new Date()).setDate(1); if (!view.visEnd) view.visEnd = new Date(); //Aaron D
			...
			for (j=0; j<levels.length; j++) {
				...
				for (k=0; k<segs.length; k++) {
					...
					if (rtl) {
						...
						left1 = startElm.position().left-5;
						//ADD - no idea why this change was necessary, but it works.
						// Firefox needs a 1 pixel correction, IE does not.
						var correction = 1;
						jQuery.each(jQuery.browser, function(i, val) {
							if(i == "msie" && val) correction = 0;
						});
//						left1 = startElm.position().left - 4 + ((seg.start.getDay()-firstDay+colCnt)%colCnt) * (td.width()+5+correction);
						
						//left2 = endElm.position().left + endElm.width();
						//ADD - the above calculation used for multi-day events, and doesn't work.
						//Since we have no multi-day events, this will do:
						left2 = left1+endElm.width() + 5;
...
function addDays(d, n, keepTime) { // deals with daylight savings
	...
	if (!keepTime) {
		clearTime(d);
	} var overflow = 0; //Aaron D
	while (d.getDate() != check.getDate() && overflow < 24) { overflow++;
